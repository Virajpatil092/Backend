...
const getDepartmentwise = async (req, res) => {
    try {
        ...
        const form = await Form.find({ department })
        res.status(200).json(form)
    } catch (error) {
        res.status(500).json(error)
    }
...
const getspecific = async (req, res) => {
    try {
        // const department = req.params.branch
        const {department, class_name } = req.body;
        const form = await Form.find({ department, class_name })
        console.log(department,class_name);
        res.status(200).json(form)
    } catch (error) {
        res.status(500).json(error)
    }
